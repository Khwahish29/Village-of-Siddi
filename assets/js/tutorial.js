...
Game.prototype.updatePopups = function () {
	...
	if (this.wood >= 10 && this.food >= 10 && !this.tutorial.tent) {
		this.showPopup(
			...
			automatically employed as lumberjacks, collecting the fallen wood for you over
	...
	if (this.levels.tent >= 1 && !this.tutorial.assign) {
		this.showPopup(
			`Now that your tent stands tall, your Siddi settlement welcomes two lumberjacks, visible in the recently unlocked "Assign" tab. Navigate to the "Craft" tab to revisit your array of available upgrades. Let's endeavor to construct a fishing pier while considering other beneficial crafts to enhance our village!`,
			"#assign",
			"assign"
		);
	...
	if (this.levels.quarry >= 1 && !this.tutorial.stone) {
		this.showPopup(
			`With the construction of the field, you can now do more Dhamaal`,
			"#warehouse"
		);
	...
	if (this.levels.smithy >= 1 && !this.tutorial.smithy) {
		this.showPopup(
			`Have you observed that the crafting process is gradually becoming more time-consuming? Assign skilled artisans to assist you with your crafts, hastening their completion significantly. The acceleration factor is displayed alongside your resource count.`,
			"#warehouse"
		);
	...
	if (this.levels.academy >= 1 && !this.tutorial.academy) {
		this.showPopup(
			`The village academy now proudly stands! A novel type of upgrade has been unveiled - researches, accessible through the freshly minted "Research" tab. You can also designate knowledgeable individuals to expedite research progress, akin to the manner in which skilled artisans quicken crafts.`,
			"#research",
			"research"
		);
